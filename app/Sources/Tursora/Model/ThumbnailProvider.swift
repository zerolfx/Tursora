...
final class ThumbnailProvider {
    ...
    static func canPreview(_ item: FileItem) -> Bool {
        !item.isNavigable && !item.isPackage
    }

    private func key(_ item: FileItem, _ size: CGFloat) -> String {
        "\(item.url.path)|\(Int(size))|\(item.modificationDate?.timeIntervalSince1970 ?? 0)|\(item.size)"
    }
    ...
    func thumbnail(for item: FileItem, size: CGFloat, scale: CGFloat,
        ...
        let request = QLThumbnailGenerator.Request(fileAt: item.url, size: CGSize(width: size, height: size),
