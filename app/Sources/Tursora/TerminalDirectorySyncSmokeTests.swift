...
enum TerminalDirectorySyncSmokeTests {
    static func run(completion: @escaping () -> Void) {
        Task { @MainActor in
            do {
                ...
                print("FAIL terminal directory sync: \(error)")
                exit(1)
            }
        }
    }
    ...
    private static func pureChecks() throws {
        ...
        try check("response preserves Unicode, newline and shell metacharacters as data", TerminalDirectorySync.parseResponse(data, token: token)?.directory.path == path)
        try check("response carries a request generation", TerminalDirectorySync.parseResponse(data, token: token)?.sequence == 4)
        ...
        try check("partial, wrong-session and malformed acknowledgements are rejected", invalid.allSatisfy { TerminalDirectorySync.parseResponse($0, token: token) == nil })
        try check("custom shells remain usable without adding zsh startup files", try TerminalDirectorySync.make(shell: "/bin/sh", environment: [:]) == nil)
        let script = TerminalDirectorySync.integrationScript(directory: URL(fileURLWithPath: "/tmp/' quoted"), token: "abc123")
        try check("integration does not inject terminal keys or claim signal handlers", !script.contains("TRAP") && !script.contains("kill ") && script.contains("zle -F -w") && script.contains("CONTEXT") && script.contains("-z \"$BUFFER\""))
        try check("wakeup and acknowledgement use builtins without spawning task processes", script.contains("builtin syswrite") && script.contains("builtin zf_mv") && !script.contains("command /bin/"))
    }
    ...
    private static func realSession() async throws {
        ...
        try check("startup retains native file order after an unset ZDOTDIR", fixture.read("order") == "env:0\nprofile\nrc\nlogin\n")
        try check("private channel directory and files have owner-only permissions", fixture.channelPermissionsArePrivate)
        try check("idle synchronization helpers are not terminal tasks", !fixture.panel.activitySnapshot().requiresConfirmation)
        ...
        try await until("idle navigation changes the real shell cwd", { fixture.read("observed") == special.path }, detail: { fixture.output })
        try check("path contents never become executable text", !FileManager.default.fileExists(atPath: special.appendingPathComponent("SHOULD_NOT_EXIST").path))
        ...
        try check("hidden synchronization retains the existing PTY", fixture.panel.terminalView === fixture.terminal && fixture.terminal.process.shellPid == originalPID && fixture.panel.isRunning)
        ...
        try await until("foreground command owns PTY before navigation", { tcgetpgrp(fixture.terminal.process.childfd) != originalPID })
        fixture.panel.followDirectory(busy)
        try await fixture.pending(keeping: hidden)
        try check("navigation cannot end the foreground command", fixture.read("busy-result") == nil)
        try await fixture.synced(to: busy)
        try check("running command keeps its cwd until completion", fixture.read("busy-result") == hidden.path)
        ...
        try await until("shell builtin read has started", { fixture.read("read-start") != nil })
        fixture.panel.followDirectory(superseded)
        fixture.panel.followDirectory(hidden)
        fixture.panel.followDirectory(reading)
        try await fixture.pending(keeping: busy)
        try check("builtin read receives no synthetic input", fixture.read("read-result") == nil && fixture.panel.isRunning)
        fixture.send("real answer ' ; $HOME\n")
        try await fixture.synced(to: reading)
        try check("read consumes exactly user input and latest navigation wins afterwards", fixture.readData("read-result") == Data("real answer ' ; $HOME\0\(busy.path)".utf8))

        let incomplete = try fixture.folder("after unfinished input")
        fixture.send("builtin print -rn -- PRESERVED > \"$HOME/incomplete-result\"")
        try await until("unfinished input is in the real line editor", { fixture.output.contains("incomplete-result") })
        fixture.panel.followDirectory(incomplete)
        try await fixture.pending(keeping: reading)
        try check("unfinished command is neither cleared nor submitted", fixture.read("incomplete-result") == nil)
        fixture.send("\n")
        try await fixture.synced(to: incomplete)
        try check("the original unfinished command still executes once submitted", fixture.read("incomplete-result") == "PRESERVED")

        let continuation = try fixture.folder("after continuation")
        fixture.send("builtin print -rn -- '\n")
        try await until("secondary prompt is active", { fixture.output.contains("MORE>") })
        ...
        try check("secondary prompt input is preserved", fixture.output.contains("__MULTILINE__"))

        let missing = fixture.root.appendingPathComponent("not created yet", isDirectory: true)
        fixture.panel.followDirectory(missing)
        try await until("failed cd is acknowledged without inventing a cwd", { fixture.panel.directorySyncUpdate?.state == .failed }, detail: { fixture.output })
        try check("failure keeps the request and real previous cwd discoverable", fixture.panel.sessionDirectory?.path == continuation.path && fixture.panel.titleLabel.toolTip?.contains("Could not synchronize") == true)
        ...
        try await until("startup-defined ZDOTDIR and trap remain accessible", { fixture.read("trap-result")?.contains("USER_TRAP") == true })
        try check("user .zshenv ZDOTDIR changes survive integration", fixture.read("zdotdir-result") == fixture.config.path)
        try await until("integration preserves the previous command exit status", { fixture.output.contains("SYNC:1>") })
        ...
        try await until("last read is awaiting input", { fixture.output.contains("LAST:") })
        fixture.panel.followDirectory(hidden)
        ...
        try await until("replaced session channel is removed", { !FileManager.default.fileExists(atPath: fixture.sync.directory.path) })
        fixture.sync.request(reading)
        try await Task.sleep(nanoseconds: 200_000_000)
        try check("replacement and invalidation reject stale requests and acknowledgements", fixture.panel.terminalView === replacement && fixture.panel.sessionDirectory?.path == special.path && fixture.panel.directorySyncUpdate == nil)
        await fixture.stop()
        try await until("replaced shell is eventually reaped", { !TerminalActivity.isAlive(originalPID) })
    }
    ...
    private static func explicitZdotdir() async throws {
        ...
        defer { fixture.dispose() }
        try await fixture.ready()
        try check("an explicit ZDOTDIR sources its own env then the updated native startup files", fixture.read("order") == "env:1\nprofile\nrc\nlogin\n")
        fixture.send("exit\n")
        try await until("natural shell exit retains ended state", { fixture.panel.statusState == .ended })
        try check("native logout startup path is preserved", fixture.read("logout") == "logout\n")
        fixture.panel.followDirectory(fixture.config)
        try check("navigation never restarts an ended shell", fixture.panel.statusState == .ended && !fixture.panel.isRunning && fixture.panel.titleLabel.toolTip?.contains("Session ended") == true)
        await fixture.stop()
    }
    ...
    private final class Fixture {
        ...
        func ready() async throws {
            ...
            try await until("real zsh primary prompt is ready", { output.contains("SYNC:0>") })
        }
        func synced(to directory: URL) async throws {
            try await until("shell acknowledges \(directory.lastPathComponent)", {
        ...
        func pending(keeping directory: URL) async throws {
            try await until("request is explicitly pending", { panel.directorySyncUpdate?.state == .waiting })
            try await Task.sleep(nanoseconds: 150_000_000)
            try check("busy or nonempty input retains the previous shell cwd", panel.sessionDirectory?.path == directory.path && panel.directorySyncUpdate?.state == .waiting && panel.titleLabel.toolTip?.contains("pending") == true)
        }
    ...
    @MainActor
    private static func until(_ name: String, _ condition: () -> Bool, detail: () -> String = { "" }) async throws {
    ...
    private static func check(_ name: String, _ value: Bool) throws {
        guard value else { throw Failure(name) }
        print("ok  terminal directory sync: \(name)")
    }
