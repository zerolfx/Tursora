...
enum SearchSmokeTests {
    ...
    @MainActor private static func runChecks() async {
        ...
        do {
            ...
            for mode: ViewMode in [.details, .icons] {
                ...
                browser.searchPanel.searchButton.performClick(nil)
                await searched(browser, label: "\(mode) Search button")
    ...
    @MainActor private static func panelActions(store: SavedSearchStore, request: SearchRequest, sourceFile: URL) {
        ...
        panel.searchButton.performClick(nil)
        check("Search button dispatches all visible conditions", searches == [request])
