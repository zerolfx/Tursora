...
Maintain truthful copy: macOS 14+, current downloadable builds for Apple Silicon, ad-hoc signing without notarization, an English application UI, and no tab restoration after quitting. Both experiments default off. ZIP browsing is read-only, opened files are temporary copies retained until app quit, and edits do not write back. The primary download links to GitHub Releases, with ZIP/checksum guidance and the private-repository access requirement. It does not claim an App Store listing or completed Finder parity.
