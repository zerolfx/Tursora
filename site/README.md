...
The product copy follows the repository About description: a free, open-source native macOS file manager and an alternative to Finder. The title and hero say **免费开源的 macOS 文件管理器**; section headings name actual features instead of abstract slogans. The Finder gap note remains visible, and the installation commands, anchors and screenshot assets are preserved. This copy describes released 0.2.0 features; the new terminal work for 0.2.1 is not presented as already released.
...
The historical installation-troubleshooting revision kept those two default commands, explicitly required the full-URL tap step to succeed first, and linked an `untrusted tap` message to README's single-cask trust instructions. Current product copy no longer offers a 0.1.0 migration path; dated release and screenshot evidence remains historical. The static build passes with five assets, 40 references, approximately 1,629 KiB and all 29 alpha checks. The two source command lines and both installation anchors are unchanged, the troubleshooting target exists, and 77 local Markdown targets pass. Evidence is `/private/tmp/tursora-tap-instructions-o5lykmq1/static-verification.json` and `site-build.{out,err}`. This check did not operate Homebrew or the browser; isolated tap/trust reproduction is recorded by the main task.
...
The flexible-installation and plain-copy revision passed the build with five assets, 41 references and all 29 screenshot alpha checks. Root browser review covered the revised homepage at 1200 × 850 and 390 × 844, the desktop navigation and narrow hero installation links, and the narrow first-launch, Homebrew and agent instructions without horizontal overflow. The installation grid aligns at the top so the longer instructions do not push the left download area downward. The guide link is labelled Markdown: the preview browser downloads it rather than rendering it. Static checks verify its copied bytes; live publication remains a separate check. Existing workflow/viewer tests retain their historical scope.
...
6. Activate both **安装指南** links and open `#installation` directly. Verify the page jumps to the guide without opening a download, the four DMG steps and Homebrew commands remain legible at 390 px, and copied commands preserve their actual three lines, in tap / single-cask trust / install order. Check the additional installer image viewer and Escape focus return. Verify that the first-launch instructions and agent prompt remain legible and that `install.md` serves the complete guide.
