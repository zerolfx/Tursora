  ...
  <tbody>
    ...
    <tr>
      <td>
        ...
        <p>Settings has General, Shortcuts, Terminal and Updates pages. General controls workspace restoration, folder views, extension display, terminal access and ZIP browsing, and can make Tursora the application macOS opens folders with, so `open .` and folders opened by other applications arrive here — it names whichever application holds that role today, and macOS asks you to confirm the change. Shortcuts lets you search all application commands, record or clear a binding, and restore one command or all defaults. Conflicts name the command already using the binding; menus update immediately. Native text editing, selection, path completion and shell controls retain their normal behavior. Updates offers daily checks, optional automatic installation and a manual check.</p>
      </td>
      <td>
  ...
  <tbody>
    ...
    <tr>
      <td>
        ...
        <p>Open a ZIP in the current pane and navigate with the address bar, Back, Forward and Up. <strong>Opening does not extract the archive</strong> — its structure is read straight away, and a file is read out of it only when you open, preview, copy, drag or share it, so even a large ZIP or a folder of thousands of files lists almost immediately. Small folders fetch their files in the background while you look at them. Use split panes, list, icon or column views, sorting, grouping and filtering. Cancel while a ZIP opens; if opening fails, retry or return to its enclosing folder. Preview, open, share, copy or drag members into regular folders.</p>
