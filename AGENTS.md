...
9. **Language.** Everything in the repository is written in English: code, comments, and every document under `docs/`, including the spec, decisions, gap lists and research records. Quoted evidence keeps its original language — a Finder or Dolphin string, a label from the Chinese product page, a fixture path — because changing it would falsify the quotation. The product website stays bilingual (`site/index.html` English, `site/zh.html` Chinese).
