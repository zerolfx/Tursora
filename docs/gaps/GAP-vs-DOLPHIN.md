...
| Dolphin 功能 | Tursora | 说明 |
|---|---|---|
| **偏好设置窗口**（通用/启动/视图模式/上下文菜单/废纸篓/确认对话框/预览） | ✅ General / Updates 与目录视图策略 | 扩展名、名称过滤快捷键、两个实验开关、每目录记忆 / 统一默认；独立更新页提供自动检查开关、可选下载安装和手动检查。更新是 Tursora 的 macOS 分发功能；其他行为策略仍待实现 |
| **本地化** | 仅英文 | 中文界面 |
