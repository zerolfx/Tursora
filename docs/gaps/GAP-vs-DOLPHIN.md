...
| Being the default file manager / "Show in Tursora" | default handler ✅; "Show in Tursora" from other apps ❌ | Settings ▸ General ▸ Opening folders claims the `public.folder` handler through `NSWorkspace.setDefaultApplication(at:toOpen:)`, so `open .`, a folder on the Dock icon and any application that asks the system to open a folder arrive in Tursora. macOS shows its own confirmation; nothing is written to LaunchServices preferences (D86). A "Reveal in Finder" item in another application calls `NSWorkspace.activateFileViewerSelecting`, which addresses Finder by name and is not redirected by the handler; there is no way to claim it |
