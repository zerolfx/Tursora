...
| **会话恢复**（记住打开的标签/pane，启动时恢复） | ✅ 已实现，待本轮验证 | 默认恢复窗口、标签顺序 / 名称 / 活动页、双 pane 位置 / 活动侧 / 比例及侧栏 / 窗口布局；可关闭并清除，已执行搜索重跑。历史、选区、过滤、终端与任务不恢复；[范围与验证](../research/workspace-sessions.md) |
...
| Dolphin 功能 | Tursora | 说明 |
|---|---|---|
| **偏好设置窗口**（通用/启动/视图模式/上下文菜单/废纸篓/确认对话框/预览） | ✅ General / Updates 与目录视图策略 | Startup 提供默认开的会话恢复开关及保存失败重试；另有扩展名、名称过滤快捷键、两个实验开关、每目录记忆 / 统一默认。独立更新页提供自动检查开关、可选下载安装和手动检查。更新是 Tursora 的 macOS 分发功能；其他行为策略仍待实现 |
| **本地化** | 仅英文 | 中文界面 |
...
- [ ] 本轮构建、连续三次 smoke、打包应用真实退出 / 重启与设置控件实测；勾选仅表示上述实现范围，完成证据见[会话恢复记录](../research/workspace-sessions.md)。
