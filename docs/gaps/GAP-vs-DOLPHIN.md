...
| Being the default file manager / "Show in Tursora" | ✅ | Settings ▸ General ▸ Opening folders claims the `public.folder` handler through `NSWorkspace.setDefaultApplication(at:toOpen:)`, so a double-clicked folder, another application's "show in enclosing folder" and `open .` all arrive in Tursora. macOS shows its own confirmation; nothing is written to LaunchServices preferences (D86). Dragging a folder onto the Dock icon still works |
