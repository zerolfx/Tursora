...
| Dolphin 功能 | Tursora | 说明 |
|---|---|---|
| **搜索**（`toggle_search`：文件名/内容，日期/类型/评分/标签 chip 筛选） | 名称 / 正文 / 类型 / 日期 / 保存条件 ✅；评分 / 标签 ❌ | 名称后台递归；正文 NSMetadataQuery，受系统索引限制。范围为当前文件夹或 Home，不搜 ZIP |
...
| Dolphin 功能 | Tursora | 说明 |
|---|---|---|
| **偏好设置窗口**（通用/启动/视图模式/上下文菜单/废纸篓/确认对话框/预览） | ✅ 基础设置 | 扩展名、名称过滤快捷键、两个实验开关；其他行为策略仍待实现 |
| **本地化** | 仅英文 | 中文界面 |
...
5. 更多信息列（Spotlight 元数据）、每目录视图属性 —— 中大
...
源码证据见 [过滤与搜索对照](../research/dolphin-filter-search.md)。
