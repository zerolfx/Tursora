...
The debug binary and the packaged app have **different UserDefaults domains** (the bare process has no bundle id), so favourites, saved searches, startup preferences and Info-section state do not carry over between them. The directory-view library and workspace session are application-owned files in Application Support, shared by ordinary debug and packaged runs. Smoke mode isolates their shared stores and uses injected temporary files for persistence checks. All application-owned UserDefaults access goes through `AppDefaults.shared`, selecting a unique suite before initialization; tests do not restore production preferences because they never write them. AppKit frame autosave and the real updater are disabled in isolated runs.
...
- `SmokeTest.run(wc)` resets the browser's view mode and group key, polls the initial model generation without blocking the main run loop (15-second deadline), then runs its named `steps` table: one entry per suite, in order, each handing the continuation to the next. `SUITE START`/`SUITE PASS` delimit each scope and report its assertion count, result-line count and elapsed time. The last step is the original navigation chain of `private static func`s; its timing closes at final run completion.
...
- `SmokeCheckGroup` retains every matrix input but prints one success summary on explicit `finish()`. A failed input exits immediately with its action/parameters. `SmokeReport` counts individual assertions separately from emitted result lines; the final run rejects unfinished groups and prints `SMOKE SUMMARY assertions=… result_lines=… suites=…` plus the five slowest suites. Count assertions from that summary rather than counting `ok` lines. Keep timer-behavior windows, but use bounded readiness conditions for ordinary thumbnail and filesystem-event completion.
- Sections that need files create them under `$TMPDIR/tursora-<suite>-…` and remove them at the end. Stop fixture measurements and detach callbacks that could persist more state, then close windows and flush their DirectoryViewPropertiesStore before removing the files. Closing a window does not immediately release its panes; a queued folder-size notification or coalesced save can otherwise recreate the fixture.
- Cached async results and their coalesced notifications have separate readiness conditions. For example, FolderSizes updates its cache before enqueueing `onUpdate` on main; a test of callback delivery must await the callback itself, not assert it immediately after observing the cache.
...
| Tab after an asynchronous completion lookup leaves focus stuck in a headless window | `selectNextKeyView` / `selectPreviousKeyView` can do nothing when the window is not key | Use `selectKeyView(following:)` / `selectKeyView(preceding:)` with the address field as the explicit source; test the actual field editor in both directions |
| Case-only rename fails on APFS, or renaming silently overwrites a sibling | `moveItem` refuses a case-only change; `URLResourceValues.name` overwrites an occupied target | Use `FileOperations.rename`, which publishes atomically with `renamex_np(RENAME_EXCL)` and preserves case-only changes |
...
| A ZIP entry not yet extracted cannot be dropped on some part of the window, though other files can (from the AppKit contract: a view only receives a drag for types it registered; the suite checks drop validation, not registration) | Such an entry is dragged as a file promise, which carries no `.fileURL`; Tursora's own drop targets recognise it by `ArchiveEntryPromiseProvider.internalType` | A new drop target registers `[.fileURL, ArchiveEntryPromiseProvider.internalType]` and reads `NSDraggingInfo.fileURLs`, never `readObjects(forClasses: [NSURL.self])` directly (D101) |
| A file opened, pasted or dropped out of a ZIP disappears from the application using it a minute after the user leaves the ZIP (a design hazard found in the D103 review, not observed) | The ZIP's private copy is let go once no pane shows it (D103), and the path handed out was inside it | Anything handed outside Tursora goes through `ArchiveHandoffStore.handOff`, never a session path — when no copy can be made, nothing is handed out and the failure is reported; work that reads the session's own files after the pane may have moved on holds `ArchiveWorkspace.lease` until it is done |
