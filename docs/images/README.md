...
`features/` contains 26 PNG screenshots from actual packaged-app or installer states. The root README uses 16 feature images in two HTML tables: 11 regular features and two terminal/ZIP features, with General and Updates sharing the settings row and a second split-workspace image showing restoration. An additional DMG installation image appears in the download section. The app icon and status badges are separate from that count. Do not use mockups as evidence of implemented features.
