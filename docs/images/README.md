...
`features/` contains 25 PNG screenshots from actual packaged-app or installer states. The root README uses 16 feature images in two HTML tables: 11 regular features and two default-off experiments, with General and Updates sharing the settings row and a second split-workspace image showing restoration. An additional DMG installation image appears in the download section. The app icon and status badges are separate from that count. Do not use mockups as evidence of implemented features.
...
4. For a staged settings capture, prefer both experiments disabled. The current `settings.png` shows both experiments off and startup restoration on, using an isolated QA app. Keep actual captured control state in the README text and alt attributes. Enable the relevant experiment for `terminal.png` and `zip-browsing.png`, then restore only preferences owned by the capture task.
