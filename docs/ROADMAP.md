...
新增功能已扩展到 ZIP 压缩 / 解压、系统分享、系统服务器挂载与设置；终端面板和 ZIP 只读浏览为默认关闭的实验功能。接下来按"用户最常碰到、成本最低"排。难度尺度（S ≤ 半天 / M 1–2 天 / L 3–5 天 / XL > 1 周）与每项的依据见两份差距清单：
...
- 推送后验证 GitHub Build 和手动 Release 的托管执行、下载产物与安装；本地打包检查不能替代实际 Actions 运行。
- 实验性终端补充不同 shell、全屏程序、字体/主题与布局检查；目前没有自动目录同步，手动 Restart 会结束当前会话。
- ZIP 只读浏览使用完整暂存，进一步评估大归档的空间与响应；密码归档、其他格式和归档写回未实现。
