...
- ZIP browsing no longer stages the whole archive: opening one mounts its table of contents and a directory's files arrive when that directory is listed (D92, [measurements](research/lazy-zip-browsing.md)). What is left: a single directory holding tens of thousands of files, or one very large member, still pays for the whole directory on entry — true per-entry deferral is a separate stage and should not be started until that case is hit in practice, because it is where the synchronous `readableContentURL` / `canAccess` / pasteboard-writer cascade actually lives. Dates come from the archive's own central directory (D93). Password-protected archives are refused by name (D91); other formats, writing back into an archive, and automatic reloading after the original ZIP changes externally are not implemented.
