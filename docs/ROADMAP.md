...
The v1 feature set (purely local) is complete: address bar, tabs, split panes, three views with zoom preview, file operations with undo, filtering, grouping, Get Info.
...
4. **Done: content search without an index (D85, 0.4.0).** `Using: Scan Files` searches file bytes where Spotlight has no index; the documented size and traversal bounds still apply.
...
- The terminal's shell / font / color settings are implemented; further coverage of more third-party shells, full-screen programs, real multi-display layouts and long-running use is still to come. zsh, bash and fish support navigation following, and shell-to-browser following shipped in 0.3.0; the implementation and limits are in the [shell sync record](research/terminal-shell-sync.md). Retention while hidden and the Restart task confirmation are in the [lifecycle record](research/terminal-session-lifecycle.md).
...
- ZIP browsing no longer stages the whole archive: opening one mounts its table of contents ([measurements](research/lazy-zip-browsing.md)); the original directory-at-a-time extraction in D92 has been superseded by the per-entry behavior below. Per-entry deferral (Stage 2) is implemented: bytes arrive through private staging with per-member attribution (D95), from a private clone of the archive (D96), and rows are built from the table of contents alone (D97). Open, Open With, Quick Look, the preview column, Copy and Copy to Other Pane extract what they need first, in the background, and thumbnails come from a transient extraction (D98–D100). Drag and Share hand out file promises and item providers for entries not yet extracted (D101), and listing a folder extracts nothing — the folder on screen fetches its small files in the background (D102). A ZIP's private copy is let go a minute after no pane shows it, and files handed to other applications are separate copies kept until quit (D103). What is left: the computer-use pass recorded as pending in the research record; browsing a ZIP nested inside another, which still opens in the default application; and quitting still removes the hand-off copies an application may be editing — the lifetime every extracted file always had — which could move to a longer-lived location with its own cleanup policy. Dates come from the archive's own central directory (D93). Password-protected archives are refused by name (D91); other formats, writing back into an archive, and automatic reloading after the original ZIP changes externally are not implemented.

## Reliability and maintenance, 2026-09-26

The current change addresses collision-safe single rename and undo, partial Trash recovery, complete external ZIP exports, Unicode listing boundaries, failed-directory row invalidation, isolated smoke preferences and asynchronous address completion. The browser now separates file commands/undo from archive reads. Implementation and verification are tracked separately in the [reliability record](research/reliability-2026-09-26.md); pending checks are not release evidence.

Smoke-suite consolidation is implemented: status geometry, common IME/model rules and archive fixtures are deduplicated, sorting checks read all three native views, and grouped output retains actual assertion totals. Named suite timings and bounded thumbnail/FSEvents readiness waits are in place. Further runtime work should follow those measurements; see the [consolidation and verification record](research/smoke-suite-review-2026-09-26.md).
...
- Broader scopes and incremental live results will be assessed later; Finder `.savedSearch` interchange, searching inside ZIPs, and ratings / tags are out of scope for this round. Unindexed content is searched by reading the files (`Using: Scan Files`, D85) rather than by building a full-text index. Both a single result set and the Spotlight candidates are capped at 50,000 items; a real positive Spotlight content-search hit is still to be verified separately; Scan Files has real-file smoke coverage.
