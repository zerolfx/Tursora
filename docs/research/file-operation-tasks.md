...
Same-volume moves, metadata system calls and the short publish transaction cannot be guaranteed to stop immediately within a single system call; Cancel takes effect at the next boundary. Items already published remain successful changes, the batch shows as partially complete, and it can be undone. Closing a browser window cancels its tasks and waits; closing the task window only hides it; quitting first cancels the transfers and then closes the archive workspace. The ZIP compress / decompress tool stages, Trash and delete are outside the range of pausable transfers.
