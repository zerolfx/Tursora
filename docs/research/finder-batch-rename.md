...
**Why not the `URLResourceValues.name` used by single-item rename**: measured (with a one-off local script), renaming `a.txt` to an already existing `b.txt` with it **does not report an error, it overwrites `b.txt`**. Batch rename has to refuse to overwrite an item of the same name outside the batch, so it uses `FileManager.moveItem` instead (which returns `NSCocoaErrorDomain 516` when the name is taken); the original claim that inline single rename had an up-front conflict check was incorrect. The 2026-09-26 reliability change replaces that unsafe primitive with `renamex_np(RENAME_EXCL)` in `FileOperations.rename`, including Info and undo/redo, while retaining case-only APFS changes. See [the follow-up](reliability-2026-09-26.md).
