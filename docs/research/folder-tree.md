...
`FolderTreeSmokeTests` 覆盖模型与窗口路径，包括辅助功能查询不加载其他目录的回归；最终 95 份 Swift 源码 3,194 项 smoke 连续三轮通过（`smoke-7` / `8` / `9`），均 exit 0、stderr 为空、源码未变。交付 app / DMG 构建与签名、包内容及安装布局核对通过；精确源码清单和组合日志见 [HANDOFF](../HANDOFF.md)。
