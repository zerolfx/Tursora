...
已更新实际截图：[分栏与独立路径](../images/features/split-panes.jpg)、[路径补全](../images/features/path-navigation.jpg)、[标签页](../images/features/tabs.jpg)。截图工具在 `NSMenu` 展开期间不可用，因此标签菜单验证依靠辅助功能树与实际点击，**没有菜单展开截图**；`tabs.jpg` 展示的是菜单关闭后的标签界面。自动覆盖的搜索 / ZIP 与生命周期边界不另声称均已完成本轮实机操作。
