...
| D30 | 2026-09-12 | 本地复制 / 移动统一为可暂停、继续、取消的独立任务；冲突在任务内异步决定，暂存完整后才发布 | 保证单个大文件可控制、任务不互相阻塞，Replace 失败不会先删旧目标。原子移动 / 元数据收尾不虚构 ETA；不扩展网络协议或 ZIP 工具控制 |
| D31 | 2026-09-12 | 传输撤销保存可逆的文件系统改名日志与私有恢复内容，绑定任务原窗口；关浏览窗口取消并等待，关标签继续，退出取消所有任务 | 只撤销真实成功项，Replace 与跨卷 Move 可安全恢复，Merge 不动无关目标内容。代价是恢复内容在撤销历史保留期间占用空间；退出前等待清理可能受文件系统响应时间限制 |
