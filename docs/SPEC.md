...
当前未实现：紧凑视图、Finder 标签读写、版本控制集成与服务菜单。终端面板和只读 ZIP 浏览默认启用，可在设置中关闭；远程连接仅使用系统挂载。
...
- 目录视图库不负责恢复窗口、标签或历史；窗口和标签由 §22 的独立会话库恢复，导航历史仍不持久化。不递归向子目录应用设置，不提供 Finder 的完整 Show View Options 对话框。Dolphin 固定源码依据与存储取舍见[每目录视图研究](research/directory-view-properties.md)；对应版本的自动与实机验证见[目录视图验证记录](research/computer-use-2026-09-12-directory-views.md)。
...
| 重命名（单项） | `Return`；或点击已选中且唯一选中行的文字，等一个系统双击间隔后进入（双击、选择变化、指针位移超过拖动阈值、拖放会话开始或结束都会取消）。预选不含扩展名的部分；大小写-only 重命名在 APFS 上可用 |
...
选中两项以上时，File 菜单、More 菜单与右键菜单的重命名项变成 Finder 的复数措辞 `Rename N Items…`，打开挂在当前窗口上的 sheet；`Return` 与单击名称仍然是单项行内重命名，不会打开 sheet。

- **Replace Text**：`Find:` / `Replace with:` 对整个名称（含扩展名）做全部替换，因此可以改扩展名；`Find` 为空时不改动。
- **Add Text**：文本 + `Where:`（`after name` / `before name`）只改主名，扩展名保留。
- **Format**：`Name Format:` 为 `Name and Index`（不补零）、`Name and Counter`（补到 5 位）或 `Name and Date`（用条目自身的修改时间，形如 `2026-09-13 at 14.02.03`），配合 `Custom Format:`、`Where:` 与 `Start numbers at:`（日期模式下隐藏）重写主名，扩展名保留。

窗口给出 `Example:` 与逐行的「旧名 → 新名」预览，随每次击键更新。名称为空、含 `/` 或 `:`、批内重复、或与**同一目录**中不参与本批的既有项重名时，`Rename` 变灰并就地显示原因（措辞取自 Finder）。批次自身让出的名字不算占用，所以连锁（a→b 同时 b→c）、互换和只改大小写都可以执行：落盘时每一项先改成同目录下的唯一临时名，再改成目标名；任一步失败整批回退，不会覆盖批外的同名文件。整批算一个撤销组，`⌘Z` 一次改回全部名称，重做再次应用；重命名后原选中项保持选中，其他 pane 与 Info 窗口按各自条目跟随新名。列表与图标视图、分栏、多标签、过滤与分组都适用；搜索结果按 URL 重命名，各自留在自己的目录里。证据、推断项与边界见[批量重命名研究](research/finder-batch-rename.md)。
...
- 按用户最后确认，底部不显示任何终端入口、图标、任务数、动画或计时，也不为此轮询进程。通过工具栏、菜单或快捷键操作终端；退出 / 关窗 / Restart 仍在动作时即时检查任务。
- 底部状态栏也移除可用磁盘容量与相应查询，保留项目 / 选中数量、过滤、搜索 / ZIP 上下文、缩放和文件操作进度；窄 pane 优先保留计数和上下文，缩放仍可通过菜单和手势操作。
...
- 浏览器导航、切换活动标签或 pane 更新终端目标目录，Settings → Terminal 的 Terminal follows the browser folder 控制是否把该目录送给正在运行的 shell（默认开）。zsh 通过私有请求文件与 ZLE FIFO 回调，在空提示符同步；忙碌、read、续行或未提交命令期间保留输入并延后到安全的提示符处理最新请求，隐藏时同样保留同步。bash 与 fish 没有唤醒通道，请求在用户画出的下一个提示符才生效，空闲时不动。路径按数据传递，不向 PTY 注入 cd 或清行按键，不修改用户 dotfiles；关闭该开关后仍更新 Restart 目标，提示说明原因。
- zsh、bash 与 fish 都在提示符汇报自己的当前目录（OSC 7，百分号编码的 `file://` URL），不需要用户自己装 shell 集成：zsh 用临时 `ZDOTDIR`（先恢复原值再读用户 `.zshenv`），bash 用临时 `--rcfile`（按登录顺序先读用户的 `/etc/profile`、`~/.bash_profile` / `~/.bash_login` / `~/.profile` / `~/.bashrc`，并追加而非覆盖 `PROMPT_COMMAND`），fish 用 `--init-command`（在 `config.fish` 之后求值）。其余 shell 仍只有手动 Restart，提示说明支持范围。启动参数全部是独立 argv，集成文件路径不进入可执行文本。
- Browser follows the shell folder（默认开）让 shell 自己换目录时，当前标签的当前 pane 跟过去，走与手动导航相同的路径，面板不读写文件系统。只在面板可见时生效；等于本面板刚请求过的目录、等于浏览器当前目录，或与上一次汇报相同的目录都不触发导航，因此两个方向不会互相拉扯。只报告本机目录（主机名须匹配本机或 localhost），远端与畸形 URL 忽略；只改活动 pane，不改另一半分栏或其他窗口，隐藏期间错过的目录不会补报。
- 顶部只有一行紧凑操作栏，保留 Terminal 标题、开始 / 重启图标和隐藏按钮；移除 Started in / Restart in 两行常驻目录文字。已确认目录、等待同步、错误和下一次启动目标放在 tooltip / 辅助功能说明中；不把启动位置当作实时 cwd。自然退出后保留输出，可按 Start Terminal 开始新会话。
...
- 本轮不提供多个终端标签，也不提供跨应用重启的 PTY 会话恢复。bash 与 fish 没有应答通道，失败的 cd 不会像 zsh 那样报告失败，只在后续提示符重试；bash 会话是交互非登录 shell。任务判断来自进程状态，不执行探测命令；检测的具体边界和验证阶段见[会话生命周期](research/terminal-session-lifecycle.md)，双向同步与 bash / fish 的范围和限制见[双向同步记录](research/terminal-shell-sync.md)。

设置实现与 Rascal 源码差异见[终端自定义](research/terminal-customization.md)；收起保留及终止确认见[会话生命周期](research/terminal-session-lifecycle.md)。0.2.1 的目录跟随、紧凑操作栏和底部简化取代旧呈现，整合与验证进度见[本轮记录](research/terminal-navigation-0.2.1.md)。 双向同步与 bash / fish 支持的范围、证据和限制见[本轮记录](research/terminal-shell-sync.md)。
...
- View → `Command Palette…`（默认 ⇧⌘O，可在设置里自定义或清除）打开一个盖在窗口上的浮动搜索面板：上方搜索框，下方结果列表。⇧⌘P 已属于 Show Previews，故不复用。
- 列表覆盖三类行：应用的全部命令（标题、所属菜单层级、以及该命令**当前**的快捷键）、侧栏 Favourites 里的每个收藏（`Go to <名称>`）、以及当前活动窗格前进 / 后退历史里的目录（`Recent: <路径>`，home 缩写为 `~`，前后各最多 12 条，去重并排除当前目录）。面板自身的命令不出现在列表里。
- 输入做子序列模糊匹配，忽略查询中的空白（`new f` 与 `newf` 等价）；排序先按得分，再按类别，再按标题。开头命中、词首命中与连续命中依次加分，断裂与起始偏移扣分，因此前缀命中必定排在同一个词出现在后面的候选之前。命中的字符在行内加粗。匹配只针对标题，不匹配类别名。
- ↑/↓ 循环移动，Return 执行，Esc 关闭；点击某行等同 Return。关闭时焦点回到文件视图。
- 当前不可用的命令**照常列出但置灰**（仍显示类别与快捷键），回车拒绝执行且面板保持打开。可用状态在面板打开的瞬间快照，执行前会再校验一次，因此不会执行已经失效的命令。
- 命令按菜单项的既有语义执行（与从菜单栏点选相同），没有菜单项的目录内命令（Rename、Quick Look、Cancel Opening Archive、下一个 / 上一个标签别名、Zoom In 别名、Select Tab 1–9）按与快捷键相同的路径执行。文件夹行导航**当前活动窗格**；分栏时只影响活动的一侧。
