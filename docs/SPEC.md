...
- 浏览器导航、切换活动标签或 pane 更新终端目标目录。默认 zsh 通过私有请求文件与 ZLE FIFO 回调，在空提示符同步目录；忙碌、read、续行或未提交命令期间保留输入并延后到安全的提示符处理最新请求，隐藏时同样保留同步。路径按数据传递，不向 PTY 注入 cd 或清行按键，不修改用户 dotfiles。其他自定义 shell 或被用户配置禁用的集成保留手动重启能力，提示说明限制；不根据终端输出驱动文件浏览器导航。
...
设置实现与 Rascal 源码差异见[终端自定义](research/terminal-customization.md)；收起保留及终止确认见[会话生命周期](research/terminal-session-lifecycle.md)。0.2.1 的目录跟随、紧凑操作栏和底部简化取代旧呈现，整合与验证进度见[本轮记录](research/terminal-navigation-0.2.1.md)。
